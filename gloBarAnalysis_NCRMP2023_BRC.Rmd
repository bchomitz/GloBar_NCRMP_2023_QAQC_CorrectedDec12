---
title: "GLOBAL NCRMP 2023 BMU Analysis"
author: "ian enochs, ben chomitz"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: yes
...
atlanticBarData_raw <- read_excel(paste(rootFolder,"/globalBar-main/analysis/data/NCRMP_Atlantic_2023_BRC_GloBar.xlsx",sep="")) #import data
pacificBarData_raw <- read_excel(paste(rootFolder,"/globalBar-main/analysis/data/NCRMP_Pacific_2023_BRC_GloBar.xlsx",sep=""))#import data
...
#Standardize data to surface area and days # changed code format
...
parrotCommunityData_atlantic_clean <- read.csv(paste(rootFolder,"/globalBar-main/analysis/data/parrotfish/parrotfish_BMUs_Atlantic.csv",sep=""), header=TRUE, sep=",") #import data
parrotCommunityData_pacific_clean <- read.csv(paste(rootFolder,"/globalBar-main/analysis/data/parrotfish/parrotfish_BMUs_Pacific.csv",sep=""), header=TRUE, sep=",")#import data
parrotCommunityData <- rbind(parrotCommunityData_atlantic_clean, parrotCommunityData_pacific_clean)
parrotFXNData_atlantic_clean <- read.csv(paste(rootFolder,"/globalBar-main/analysis/data/parrotfish/parrotfishFXN_BMUs_Atlantic.csv",sep=""), header=TRUE, sep=",")#import data
parrotFXNData_pacific_clean <- read.csv(paste(rootFolder,"/globalBar-main/analysis/data/parrotfish/parrotfishFXN_BMUs_Pacific.csv",sep=""), header=TRUE, sep=",")#import data
...
environmentalData <- read.csv(paste(rootFolder,"/globalBar-main/analysis/data/environmental/eds_time.csv",sep=""), header=TRUE, sep=",") #import data
environmentalData<-environmentalData%>%
  mutate(Degree_Heating_Weeks_CRW_daily_01dy =(case_when(
    Degree_Heating_Weeks_CRW_daily_01dy == "Inf"  ~ NA)))%>%
  mutate(major_Degree_Heating_Weeks_CRW_daily_01dy =(case_when(
    major_Degree_Heating_Weeks_CRW_daily_01dy == "Inf"  ~ NA)))%>%
  mutate(loc3 =(case_when(
    lat == 18.48227  ~ "arecibo", 
    lat == 18.33066  ~ "culebra",
    lat == 18.33696  ~ "fajardo",
    lat == 17.91047  ~ "jobosBay",
    lat == 17.95355  ~ "laParguera",
    lat == 18.34435  ~ "rincon",
    lat == 17.82805  ~ "langBank",
    lat == 17.78532  ~ "saltRiver",
    lat == 17.70121  ~ "south",
    lat == 17.73380  ~ "spratHole",
    lat == 18.36316  ~ "johnsonsReef",
    lat == 18.34744  ~ "newfound",
    lat == 18.32802  ~ "stevenCay",
    lat == 18.30976  ~ "tektite",
    lat == 18.34402  ~ "brewersBay",#note only one brewersBay on environmental but multiple on BMU
    lat == 18.38262  ~ "innerBrass",
    lat == 18.34082  ~ "sava",
    lat == 24.61123  ~ "birdKeyReef",
    lat == 24.58590  ~ "LKI1",
    lat == 24.57899  ~ "LKI2",
    lat == 24.52202  ~ "LKO1",
    lat == 24.55326  ~ "LKO2",
    ...
    lat == 24.90250  ~ "UKO1",
    lat == 24.88266  ~ "UKO3",
    lat == 24.89742  ~ "cheecaRocks",
    lat == 27.90740  ~ "eastBank",
    lat == 13.60334  ~ "OCC-GUA-002",
    lat == 13.47516  ~ "OCC-GUA-005"))) #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<NEED TO EXPAND THESE OUT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
```
...
siteEnvironmentalStats <- environmentalData %>%
  ...
  group_by(basin, loc1, loc2, loc3, lat, lon) %>%
...
barSiteMap <-leaflet()%>%
  ...
  addCircleMarkers(data = environmentalData,
             radius = 1,
             color = "green",
             popup = ~loc2,
             lng = ~lon,
...
grazingVsDeltaMassPlot <- ggplot(allBarData_grazingAnalysis, aes(x = deltaMass_mgcm2y, y = grazing_mm3cm2y)) +
  ...
  geom_smooth(method = "lm", se = TRUE) +  # Regression line
  #geom_text(aes(x = deltaMass_mgcm2y, y = grazing_mm3cm2y, label = tagNum)) + # unhash to identify outlier tag numbers
  theme_minimal()
